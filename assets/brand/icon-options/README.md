...
```bash
# App icon (opaque)
npx @resvg/resvg-js-cli assets/brand/icon-options/icon-dark.svg assets/icon.png --fit-width 1024

# Android adaptive icon + splash (transparent foreground)
npx @resvg/resvg-js-cli assets/brand/icon-options/icon-dark-transparent.svg assets/adaptive-icon.png --fit-width 1024
npx @resvg/resvg-js-cli assets/brand/icon-options/icon-dark-transparent.svg assets/splash-icon.png --fit-width 1024
```

### Android adaptive icon
`icon-dark-transparent.svg` is the foreground-only source (no opaque background). The system background colour `#080B12` is provided by `app.json`. Export directly to `assets/adaptive-icon.png` using the command above.
