...
The AI assistant writes its question(s) as normal prose, then appends a hidden HTML comment containing a JSON payload.
...
<!-- clawboy:options
{
  "questions": [
    {
      "id": "agent_id",
      "prompt": "Agent id — pick now, can't easily rename",
      "choices": [
        { "label": "twinkle",  "value": "twinkle" },
        { "label": "glow",     "value": "glow" },
        { "label": "partner",  "value": "partner" },
        { "label": "guest1",   "value": "guest1" }
      ],
      "allowFreeText": true,
      "freeTextPlaceholder": "Or her actual name / nickname…"
    },
    {
      "id": "workspace_dir",
      "prompt": "Workspace dir name — match the agent id or different?",
      "choices": [
        { "label": "Match agent id", "value": "match" },
        { "label": "Different",      "value": "different" }
      ]
    },
    {
      "id": "user_md",
      "prompt": "USER.md stub or blank? Default = blank.",
      "choices": [
...
```
<!-- clawboy:answers
{"agent_id":"twinkle","workspace_dir":"match","user_md":null}
-->
...
- The hidden HTML comment carries the machine-readable JSON keyed by `question.id`. `null` = skipped.
- The visible numbered list uses each question's `prompt` and either the chosen `choice.label`, the freeform text, or `(skipped)`.
- The comment is invisible to all markdown renderers — Discord, web UI, Telegram, etc. see only the numbered list.
...
1. Parsing any `<!-- clawboy:answers {...} -->` block in the next user message (precise, per-id).
2. Falling back to the legacy label/value match for old single-question history that predate the answers directive.
...
Version: `CONVENTION_VERSION = 5` — v5 compacts the primer ~35% and adds the explicit framing that interactive cards are the primary way to ask the user questions in ClawBoy. Triggers `AGENTS.md` reinstall on next session open.
...
Append a hidden HTML comment after your prose:
...
<!-- clawboy:options
{"questions":[
  {"id":"q1","prompt":"First question?","choices":[{"label":"A","value":"a"},{"label":"B","value":"b"}]},
  {"id":"q2","prompt":"Second question?","choices":[{"label":"Yes","value":"yes"},{"label":"No","value":"no"}]}
]}
-->
