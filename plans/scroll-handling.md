...
- `sendAnchorHeld` (state + `sendAnchorHeldRef`) — latched true when send-anchor's `scrollToOffset` fires for a new user-message tail. Forces `rawNeedsAnchorSpace` to stay true past stream-end so the bottom spacer + MVCP-off gate persist after a short reply, preventing the iOS UIScrollView clamp-to-maxOffset that would otherwise snap the reply to the bottom (the 2-stage rAF cascade alone is not sufficient — it prevents the JS-driven MVCP snap, not the native UIScrollView clamp on contentH shrink). Cleared by: (a) `scrollToBottom` (pill tap), (b) `sessionKey` effect (session swap), (c) `onScroll` when `distFromEnd >= 0` AND `!sendAnchorPending` — i.e. the user has scrolled enough that the assistant reply's bottom has reached or crossed the viewport bottom edge, (d) at the top of the send-anchor effect on every new tail user message — the prior send's held anchor is "consumed" by the new send so the cascade re-evaluates fresh; the new send re-arms the latch in RAF 3 via `setSendAnchorHeld(true)`. The clear at (d) is safe because `lastIsUser=true` keeps `rawNeedsAnchorSpace` true synchronously on the same render — spacer never collapses, no iOS clamp window opens during the transition. Without (d), a stale latch from msg A could keep the cascade in an inconsistent state when msg B is appended, producing the 2nd-send misanchor described in §12 Bug #13. The geometry at the release moment (for path c) is self-aligning: current `offsetY == realContentH - layoutH == post-collapse maxOffset`, so the spacer collapse is seamless (no iOS clamp). Drag does NOT clear directly — small drags would otherwise release the anchor and immediately re-trigger the clamp we're avoiding. See §6.
...
- Consecutive sends (send msg A → short reply → send msg B while `sendAnchorHeld` is still latched from msg A) anchor msg B at the top of viewport with context band, identically to the 1st send. The send-anchor effect clears the stale `sendAnchorHeld` at entry (path d in §4) so the cascade re-evaluates fresh against msg B; `lastIsUser=true` keeps `rawNeedsAnchorSpace` true so the spacer never collapses. Without this clear, the stale latch could leak into the 2nd-send transition and the msg B append could land at viewport bottom with its reply streaming off-screen below. See Bug #13.
...
- [ ] Consecutive sends — send msg A → wait for short reply (sendAnchorHeld latches) → immediately send msg B → msg B anchors at viewport top with context band, NOT at viewport bottom; reply 2 streams in visibly below msg B (Bug #13 regression check)
...
### Bug #13 — 2nd consecutive send snaps to viewport bottom (composer-focus corrective Path A overrides send-anchor)

**Status:** FIXED. Root cause was `correctiveEndScroll` Path A firing `scrollToMessagesEnd(false)` (animated:false) on keyboard worklet `onEnd` while send-anchor's `scrollToOffset(target, animated:true)` was in flight. animated:false beats animated:true on iOS UIScrollView, so the corrective wins and msg B lands at `maxOffset = contentH - spacer - layoutH` (i.e., viewport bottom). Reply 2 then streams in below the fold. Confirmed in instrumented logs: `[KB] corrective Path A scrollToMessagesEnd` fires ~16ms after `[SendAnchor] SET sendAnchorHeld=true`, and the resulting `[Scroll] offsetY → 1478` exactly matches `2534 - 528 - 528`.

**Symptom (pre-fix):** Mid-conversation. User taps input bar → keyboard rises → composer focus flag set. User sends msg → send-anchor effect computes target near viewport top, kicks off `scrollToOffset(target, animated:true)`. Concurrently, keyboard worklet `onEnd` (fires when kb stabilizes after any layout perturbation, e.g. the brief shrink/grow on text submit) calls `correctiveEndScroll`. The corrective's Path A branch (`composerFocusFlagRef && isNearBottomRef`) fires `scrollToMessagesEnd(false)` — an unconditional anchor-to-tail scroll meant for "user just focused composer, settle tail at bottom." The animated:false snap overrides the in-flight send-anchor animated scroll. Msg B lands at viewport bottom, agent reply for msg B streams in below the visible window.

**Root cause:** [MessageList.tsx:1700](src/components/chat/MessageList.tsx#L1700) — `correctiveEndScroll`'s Path A branch was unconditional on `composerFocusFlagRef + isNearBottomRef`. The flag stays set from `handleComposerFocus` until the worklet's `onEnd` clears it via `clearComposerFocusFlag`. When the user sends a message while the flag is still set, both send-anchor and corrective race; corrective's animated:false wins. Per-frame Path A in `onKeyboardFrame` was already guarded against this via `needsAnchorSpaceRef` ([MessageList.tsx:1605-1610](src/components/chat/MessageList.tsx#L1605-L1610)), but `correctiveEndScroll` ran outside that guard and had no equivalent.

**Prescribed fix:**

- `MessageList.tsx` `correctiveEndScroll` — gate Path A on `!sendAnchorPendingRef.current && !sendAnchorHeldRef.current`. If either is true, send-anchor owns scroll for the current cycle, and corrective Path A must not override.
- Secondary preventive fix already in place (kept for defense-in-depth): clearing stale `sendAnchorHeld` at the top of the send-anchor effect so the latch is always re-evaluated per new tail user msg.
- Diagnostic `[SendAnchor]` / `[KB] corrective` logs preserved gated on `EXPO_PUBLIC_DEBUG_LIST_PERF=1` / `EXPO_PUBLIC_DEBUG_KEYBOARD=1` for any future race regressions in this area.

**Files modified:** `src/components/chat/MessageList.tsx`

**Non-regression:**
- Composer-focus tail anchor (the legitimate Path A scenario): preserved. When user taps input bar without sending (`sendAnchorPending=false && sendAnchorHeld=false`), corrective still fires `scrollToMessagesEnd(false)` as before.
- Bug #9 (short-reply no-snap): preserved. Send-anchor's latch lifecycle untouched.
- Bug #12 (first send anchor): preserved. First send has `sendAnchorPending=false` going in but flips true synchronously inside the effect at `setSendAnchorPending(true)`; by the time corrective runs (after kb stabilizes), pending is true → corrective skipped → send-anchor wins.
- §9 "Empty/short conversation": preserved.
- "No auto-follow during streaming": preserved (corrective Path A was never the auto-follow path).
- Annotation Path B (`pendingRevealRef`): preserved. Path B's fallback in correctiveEndScroll runs BEFORE the new send-anchor gate (in the `if (p) { ... return; }` branch).

**Verification:**
- Mid-conv with composer focused: send msg A → wait for assistant reply (any length) → send msg B → msg B anchors at viewport top with context band (NOT at viewport bottom). Repeat with rapid consecutive sends.
- Tap input bar (focus composer) without sending → keyboard rises → list anchors tail at bottom in one smooth motion. Confirms Path A still works for its legitimate trigger.
- With `EXPO_PUBLIC_DEBUG_KEYBOARD=1`, after send: should see `[KB] corrective Path A SUPPRESSED (send-anchor active)` if corrective tries to fire during the send-anchor window. Without the suppression, `[KB] corrective Path A scrollToMessagesEnd` would fire and the bug would reproduce.
